...
public class TakedaSamuraiArmorItem extends GeoArmorItem implements IAnimatable {
    private final AnimationFactory factory = new AnimationFactory(this);



    public TakedaSamuraiArmorItem(ArmorMaterial materialIn, EquipmentSlot slot, Item.Settings builder) {
    ...
    private <P extends IAnimatable> PlayState predicate(AnimationEvent<P> event) {
        ...
        if (livingEntity instanceof PlayerEntity) { // <-- if the living entity is a player
            ...
            if (!(player.lastLimbDistance > -0.15F && player.lastLimbDistance < 0.15F)) { //<--- Check if the players limbs are moving
                ...
                event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.takeda_samurai_armor.run", true)); //<-- get the animation, true to loop
                return PlayState.CONTINUE;  //<-- play the animation continously
            }


        }
        event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.takeda_samurai_armor.idle", true)); //<-- if the players libs are not moving, use the idle animation (which doesn't do anything)
